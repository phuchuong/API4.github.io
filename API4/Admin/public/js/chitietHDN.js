...
app.controller('chitiethoadonnhap', function($scope, $http) { //tao 1 controller
    ...
    $http({
        method: "GET",
        url: "http://localhost:8000/api/chitietHDN"
    }).then(function(response) {
        console.log(response.data);
        $scope.chitietHDN = response.data;
    });
    //loasd lai trang
    ...
    $scope.showmodal = function(id) {
        ...
        if (id == 0) {
            ...
            $http({
                method: "GET",
                url: "http://localhost:8000/api/chitietHDN/"+ id
            }).then(function(response) {
    ...
    $scope.saveData = function() {
        if ($scope.id == 0) { //dang them moi sp
            ...
            $http({
                method: "PUT",
                url: "http://localhost:8000/api/chitietHDN/" + $scope.id,
                data: $scope.contain,
                "content-Type": "application/json"
            }).then(function(response) {
                $scope.message = response.data;
